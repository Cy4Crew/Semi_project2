...
def _allowed_extensions() -> set[str]:
    return {x.strip().lower() for x in settings.allowed_upload_extensions.split(",") if x.strip()}


def _magic_hex_prefixes() -> list[bytes]:
    out = []
    for item in str(getattr(settings, "allowed_upload_magic_hex", "") or "").split(","):
        item = item.strip().lower().replace("0x", "")
        if not item:
            continue
        try:
...
def _magic_bytes_allowed(data: bytes) -> bool:
    prefixes = _magic_hex_prefixes()
    if not prefixes:
        return True
...
async def upload_sample(file: UploadFile = File(...)):
    record_event("upload_received", "sample upload received", payload={"filename": file.filename or "sample.zip"})
    safe_name = _safe_upload_name(file.filename)
    if Path(safe_name).suffix.lower() not in _allowed_extensions():
        raise HTTPException(status_code=400, detail="Only approved upload extensions are allowed.")
    ...
    if not _magic_bytes_allowed(data[:16]):
        raise HTTPException(status_code=400, detail="Upload magic bytes do not match allowed archive signatures.")

    sample_path = _unique_sample_path(safe_name)
    sample_path.write_bytes(data)
    record_stage_metric(None, "upload", "done", duration_ms=0, payload={"filename": safe_name, "size": len(data)})
    logger.info("Stored uploaded sample %s", safe_name)
