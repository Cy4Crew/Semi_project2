    ...
    function renderSummaryPanel(data) {{
      ...
      return `
        <div class="kv">
          ...
          <div class="mini-title">Primary type</div><div>${{esc(data.primary_malware_type || '-')}}</div>
          <div class="mini-title">Type tags</div><div>${{renderList(data.malware_type_tags || [], 'No type tags')}}</div>
          <div class="mini-title">Per-file scoring</div><div>Each archive member gets final score, executed flag, fail reason, top evidence, and reverse rule in the detail page.</div>
