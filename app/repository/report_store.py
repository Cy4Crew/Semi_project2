...
def _sample_sha256(payload: dict[str, Any]) -> str:
    hashes = payload.get("sample_hashes") or {}
    return str(hashes.get("sha256") or "").strip().lower()
...
def _upsert_payload(conn: sqlite3.Connection, payload: dict[str, Any]) -> None:
    ...
    conn.execute(
        """
        INSERT INTO reports(report_id, status, filename, verdict, risk_score, sample_sha256, updated_at, created_at, payload_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def list_reports(limit: int = 30, offset: int = 0) -> list[dict[str, Any]]:
    ensure_report_store()
    with _REPORT_LOCK, _connect() as conn:
        rows = conn.execute(
            "SELECT payload_json FROM reports ORDER BY updated_at DESC, report_id DESC LIMIT ? OFFSET ?",
...
def find_cached_report_by_sha256(sample_sha256: str, allowed_statuses: list[str] | None = None) -> dict[str, Any] | None:
    ensure_report_store()
    sample_sha256 = str(sample_sha256 or "").strip().lower()
    if not sample_sha256:
        return None
    allowed = [str(s).strip().lower() for s in (allowed_statuses or []) if str(s).strip()]
    query = "SELECT payload_json FROM reports WHERE sample_sha256 = ?"
    params: list[Any] = [sample_sha256]
    if allowed:
